...
//In questa implementazione prenderemo ad esempio un albero 3-D
...
{ 
	Node *res = x; 
	if (y != NULL && y->coordinates[d] < res->coordinates[d]) //Il confronto avviene sull'asse d dato in input
	res = y; 
...
{ 
	...
	//Se la dimensione del livello corrente è uguale alla dimensione data, il minimo richiesto si trova sul lato sinistro se esiste
	if (cd == d) 
	{ 
		if (root->left == NULL) 
			return root; 
...
{ 
	...
	cout<<"("<<minimo->coordinates[0] <<","<<minimo->coordinates[1]<<")"<<endl;
	root = deleteNode(root, points[0]); 
	cout << "La radice dopo la cancellazione di  (5,4) è "; 
	for(int i=0;i<dimensione;i=i+2)
		cout<<"("<<root->coordinates[0]<<","<<root->coordinates[i+1]<<")"<<endl;
	...
	for(int i=0;i<dimensione;i=i+2)
		cout<<"("<<target->coordinates[i]<<","<<target->coordinates[i+1]<<") ";
	cout<<"ha coordinate ";
	for(int i=0;i<dimensione;i=i+2)
		cout<<"("<<NN->coordinates[i]<<","<<NN->coordinates[i+1]<<")"<<endl;
	return 0; 
} 
